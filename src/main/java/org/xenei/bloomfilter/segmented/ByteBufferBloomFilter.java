...
public class ByteBufferBloomFilter implements BloomFilter {
    ...
    public ByteBufferBloomFilter(Shape shape, ByteBuffer buffer, int position) {
        this.buffer = (buffer == null) ? ByteBuffer.allocate(numberOfBytes(shape.getNumberOfBits())) : (position == 0) ? buffer.duplicate() : buffer.slice();
        this.shape = shape;
        ...
        return indexProducer.forEachIndex(i -> {
            if (i >-1 && i < shape.getNumberOfBits()) {
                ...
                throw new IllegalArgumentException( 
                        String.format("Bloom filter only accepts indexes in the range [0..%s)", shape.getNumberOfBits()));
            }
        });
    }

 ...
 * 
 * The hash is calculated as a 128-bit value.  We store this as two 64-bit values.
 * We can then rapidly calculate the bloom filter for any given configuration.
 *
 */
class Hash implements Comparable<Hash> {
	...
	public BitSet populate( BitSet set, FilterConfig config )
	{
		...
		{
			throw new IllegalArgumentException( String.format("Bitset had %s bits, %s required", set.size(), config.getNumberOfBits()));
		}

...
public final class MurmurHash3 {
    ...
    public static final long getLongLittleEndian(ByteBuffer buf, int offset) {
        return ((long) buf.get(offset + 7) << 56) // no mask needed
                | ((buf.get(offset + 6) & 0xffL) << 48) | ((buf.get(offset + 5) & 0xffL) << 40)
                | ((buf.get(offset + 4) & 0xffL) << 32) | ((buf.get(offset + 3) & 0xffL) << 24)
                | ((buf.get(offset + 2) & 0xffL) << 16) | ((buf.get(offset + 1) & 0xffL) << 8)
    ...
    public static int murmurhash3_x86_32(ByteBuffer data, int offset, int len, int seed) {
        ...
        for (int i = offset; i < roundedEnd; i += 4) {
            ...
            int k1 = (data.get(i) & 0xff) | ((data.get(i + 1) & 0xff) << 8) | ((data.get(i + 2) & 0xff) << 16)
                    | (data.get(i + 3) << 24);
            k1 *= c1;
